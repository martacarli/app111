...
test('selectClosestOptions: the overage bound is symmetric — undershooting the target is bounded too, not just overshooting', () => {
  // Regression: withinBound used to only check distanceMeters <= target +
  // maxOverageMeters (a one-sided cap on going over), so a candidate way
  // SHORTER than the target had no floor at all and still counted as
  // "within bound." A 1000m bound around a 5000m target should mean
  // [4000, 6000], not "anything below 6000."
  const pool = [
    candidate(4200, 0.0, 'a', 1), // clean, within [4000, 6000]
    candidate(2000, 0.0, 'b', 1), // clean, but way undershoots — 3000m under target
  ];
  const options = selectClosestOptions(pool, 5000, 1000, 2);
  assert.equal(options[0].exceedsPreferredBound, false);
  assert.equal(options[1].exceedsPreferredBound, true); // 2000 is 3000m short, outside the ±1000m window
});
