...
export function computeRunLogSummary(log) {
  return log.reduce(
    (summary, entry) => ({
      totalRuns: summary.totalRuns + 1,
      totalDistanceMeters: summary.totalDistanceMeters + (entry.distanceMeters ?? 0),
      totalDurationSeconds: summary.totalDurationSeconds + (entry.durationSeconds ?? 0),
    }),
    { totalRuns: 0, totalDistanceMeters: 0, totalDurationSeconds: 0 }
  );
}
